...
interface EditCustomerGSTModalProps {
...
export function EditCustomerGSTModal({ customerGST, onClose, onSuccess }: EditCustomerGSTModalProps) {
  const [loading, setLoading] = useState(false);
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [states, setStates] = useState<State[]>([]);
  const [formData, setFormData] = useState({
    customer_code: customerGST.customer_code,
    customer_name: customerGST.customer_name,
    gstin: customerGST.gstin || '',
    bill_to_address: customerGST.bill_to_address,
    state_id: customerGST.state_id || '',
    state_code: customerGST.state_code || '',
    alpha_code: customerGST.alpha_code || '',
    is_active: customerGST.is_active,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', customerGST.id);

      if (error) throw error;

      alert('Customer GST record updated successfully!');
      onSuccess();
    } catch (error: any) {
      console.error('Error updating customer GST record:', error);
      alert(error.message || 'Failed to update customer GST record');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Edit Customer GST</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
            ...
            >
              {loading ? 'Updating...' : 'Update Customer GST'}
            </button>
          </div>
        </form>
      </div>

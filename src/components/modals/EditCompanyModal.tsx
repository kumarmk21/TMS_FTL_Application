...
export function EditCompanyModal({ company, onClose, onSuccess }: EditCompanyModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    msme_no: (company as any).msme_no || '',
    pan: (company as any).pan || '',
    bank_name: (company as any).bank_name || '',
    account_number: (company as any).account_number || '',
    ifsc_code: (company as any).ifsc_code || '',
    bank_branch: (company as any).bank_branch || '',
    bill_footer1: (company as any).bill_footer1 || '',
    bill_footer2: (company as any).bill_footer2 || '',
    bill_footer3: (company as any).bill_footer3 || '',
    bill_footer4: (company as any).bill_footer4 || '',
  });
  ...
  const uploadLogo = async (): Promise<string | null> => {
    ...
    try {
      ...
      const fileName = `logos/${company.company_code}_${Date.now()}.${fileExt}`;
      ...
      <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b sticky top-0 bg-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold">Edit Company - {company.company_code}</h2>
          </div>
          <button
            onClick={onClose}
                ...
                <label className="flex-1 cursor-pointer">
                  <div className="flex items-center justify-center gap-2 px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-500 transition-colors">
                    ...
                    <span className="text-sm text-gray-600">
                      {logoFile ? logoFile.name : 'Upload New Logo (Max 5MB)'}
                    </span>
                  </div>
                  <input
                    type="file"

...
interface EditWarehouseBillModalProps {
  billId: string;
  onClose: () => void;
  onSuccess: () => void;
}

export function EditWarehouseBillModal({ billId, onClose, onSuccess }: EditWarehouseBillModalProps) {
  ...
  }, [formData.bill_sub_date, formData.credit_days, initialLoad]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (billData.data) {
        ...
        const customerGSTRecords = gstData.filter(gst => gst.customer_code === bill.billing_party_code);
        ...
        setFormData({
          ...
          billing_party_id: bill.billing_party_id || '',
          ...
          service_branch_id: bill.service_branch_id || '',
          service_branch_name: bill.service_branch_name || '',
          ...
          bill_status: bill.bill_status || 'Draft',
          tds_applicable: bill.tds_applicable || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (updateError) {
        console.error('Error updating warehouse bill:', updateError);
        alert(`Error updating warehouse bill: ${updateError.message}`);
        setLoading(false);
        return;
      }
      ...
      console.error('Error updating warehouse bill:', error);
      alert(`Error updating warehouse bill: ${error?.message || 'Please try again.'}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[95vh] overflow-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-xl font-semibold text-gray-900">Edit Warehouse Bill</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="border-t pt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Charges & Tax Calculation</h3>
